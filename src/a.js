...
const WEBHOOK_PORT = Number(process.env.WEBHOOK_PORT || "3001"); // bisa diubah
const CS_CONTACT = process.env.CS_CONTACT || "Hubungi admin: wa.me/62xxxxxxxxxx";
...
// Timestamp WIB (YYYY-MM-DD HH:mm:ss WIB)
function formatTs(ts) {
  if (!ts) return "-";
  const dt = new Date(ts);
  const pad = n => String(n).padStart(2, "0");
  // pakai opsi timezone untuk konsisten WIB
...
// Menu rapi dengan garis pembatas
function prettyMenu(){
  const header = [
  ...
  const items = PRODUCTS.map((p, idx) => {
    ...
    return idx === PRODUCTS.length - 1 ? block : (block + "\n\n" + DIV + "\n");
  }).join("\n\n");

  return header + "\n" + items + "\n\n_✍️ Ketik: beli <kode> <jumlah>_";
}

// Help yang lebih jelas + contoh
function userHelp() {
  return [
    ...
    "• 💳 `bayar <idorder>` — *opsional*, kalau ingin konfirmasi manual",
    "• 🔎 `cekorder <id>` — lihat detail 1 order",
    ...
    "3) Scan QR & bayar *sejumlah yang tertulis* (sudah ada kode unik)",
    "4) Verifikasi otomatis ✅ (nggak perlu kirim *bayar*)",
  ].join("\n");
}
...
let webhookStarted = false; // agar server hanya start sekali
function startPaymentWebhook(sock, send){
  ...
  try {
    ...
    // Tangani EADDRINUSE agar tidak crash
    server.on("error", (err) => {
...
async function startSock() {
  ...
  // === Mulai webhook auto-verify (start sekali; aman dari EADDRINUSE) ===
  ...
  async function handleMessage(mUp){
    try{
      ...
      if (lower === "orders") {
        ...
        const out = list.map(o => `• ${o.id} — ${o.status} — ${formatPrice(o.total)}${o.expectedAmount ? ` (tagihan: ${formatPrice(o.expectedAmount)})` : ""} — ${formatTs(o.createdAt)}`).join("\n");
      ...
      if (isAdmin) {
        ...
        if (lower.startsWith("refund ")) {
          ...
          // Restock hanya jika belum dikirim
